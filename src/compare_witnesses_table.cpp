 ...
 * Default constructor.
 */
compare_witnesses_table::compare_witnesses_table() {

}

/**
 * Constructs a compare witnesses table relative to a given witness,
 * given a list of witness IDs in input order and a set of witness IDs by which to filter the rows.
 */
 compare_witnesses_table::compare_witnesses_table(const witness & wit, const list<string> list_wit, const set<string> & filter_wits) {
    rows = list<compare_witnesses_table_row>();
    id = wit.get_id();
    ...
    for (string secondary_wit_id : list_wit) {
        ...
        compare_witnesses_table_row row;
    ...
    rows.sort([](const compare_witnesses_table_row & r1, const compare_witnesses_table_row & r2) {
        return r1.eq > r2.eq;
    });
    ...
    for (compare_witnesses_table_row & row : rows) {
	...
	if (!filter_wits.empty()) {
		rows.remove_if([&](const compare_witnesses_table_row & row) {
			return filter_wits.find(row.id) == filter_wits.end();
		});
	}
}
 ...
 */
string compare_witnesses_table::get_id() const {
    return id;
}
 ...
 */
int compare_witnesses_table::get_primary_extant() const {
 ...
 */
list<compare_witnesses_table_row> compare_witnesses_table::get_rows() const {
    return rows;
}

/**
 * Given an output stream, prints this witness comparison table in fixed-width format.
 */
void compare_witnesses_table::to_fixed_width(ostream & out) {
    //Print the caption:
...
/**
 * Given an output stream, prints this witness comparison table in comma-separated value (CSV) format.
 * The witness IDs are assumed not to contain commas; if they do, then they will need to be manually escaped in the output.
 */
void compare_witnesses_table::to_csv(ostream & out) {
    //Print the header row:
	out << "W2" << ",";
	out << "DIR" << ",";
	out << "NR" << ",";
...
/**
 * Given an output stream, prints this witness comparison table in tab-separated value (TSV) format.
 * The witness IDs are assumed not to contain tabs; if they do, then they will need to be manually escaped in the output.
 */
void compare_witnesses_table::to_tsv(ostream & out) {
    //Print the header row:
	out << "W2" << "\t";
	out << "DIR" << "\t";
	out << "NR" << "\t";
